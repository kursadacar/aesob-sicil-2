...
var locPath = "/" +
    splitPath[1] +
    (splitPath[2] != undefined ? "/" + splitPath[2] + (splitPath[3] != undefined ? "/" + splitPath[3] : "") : "");
$.ErcanAyhan.LeftSideBar = {
    activate: function() {
        ...
        if (activeMenu.length != 0) {
            activeMenu.addClass(
                (activeMenu.parent().parent().parent().find("i")[0].className.replace("material-icons", "")));
            ...
            activeMenu.parent().parent().parent().find("span")[0].className += (activeMenu.parent().parent().parent()
                .find("i")[0].className.replace("material-icons", ""));
            activeMenu.parent().parent().parent().parent().parent().find("span")[0].className += (activeMenu.parent()
                .parent().parent().parent().parent().find("i")[0].className.replace("material-icons", ""));
            if (activeMenu.parent().find("i")[0] != undefined)
                activeMenu.parent().find("span")
                    .addClass(activeMenu.parent().find("i")[0].className.replace("material-icons", ""));
        }
...
$.ErcanAyhan.DropdownMenu = {
    ...
    dropdownEffect: function(target) {
        var effectIn = $.ErcanAyhan.options.dropdownMenu.effectIn,
            effectOut = $.ErcanAyhan.options.dropdownMenu.effectOut;
...
$.ErcanAyhan.ToTitleCase = function(txt) {
    if (txt != null) {
        ...
        for (var i = 0; i < splitTxt.length; i++) {
            text += splitTxt[i].charAt(0).turkishToUpper() +
                splitTxt[i].substr(1).turkishToLower() +
                (i + 1 == splitTxt.length ? "" : " ");
        }
        return text;
    } else
    ...
    PostAsync: function(model, url, func) {
        ...
        $.ajax({
            ...
            error: function(xhr, errorType, exception) {
                ...
                if (xhr.statusText == "warning" ||
                    xhr.statusText == "info" ||
                    xhr.statusText == "success" ||
                    xhr.statusText == "danger" ||
                    xhr.statusText == "func" ||
                    xhr.statusText == "funcAndMessage") {
...
//onkeyup="tarih(this);"  
function tarih(alanAd) {
    var str = alanAd.value;
    var yer = "";
    if (alanAd.value.length == 2) {
        yer = alanAd.value;
        yer = yer + ".";
        alanAd.value = yer;
    } else if (alanAd.value.length == 5) {
        yer = alanAd.value;
        yer = yer + ".";
        alanAd.value = yer;
    }
...
//onkeypress="return Numeric(this,event);"
...
//onkeyup="BuyukHarf(this)"
function BuyukHarf(gelen) {
    var turkce = gelen.value;
    len = turkce.length;
    var i = 1;
    while (i < len + 1) {
        turkce = turkce.replace("i", "İ");
        i = i + 1;
    }
    gelen.value = turkce.toUpperCase();
}
...
function bugun() {
    ...
    var bugun2 = tarih + "." + aylar[zaman.getMonth()] + "." + (cevir(zaman.getYear()));
    return bugun2;
}